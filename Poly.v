...
(** **** Exercise: 1 star, standard, optional (combine_checks)
    ...
    - What is the type of [combine] (i.e., what does [Check
      ...
      [(X : Type) -> (Y : Type) -> list X -> list Y -> list (X*Y)]
...
Lemma map_distr : forall (X Y : Type) (f : X -> Y) (l : list X) (x : X),
...
*)
(*
  Theorem: Given a type [X], a list [l] of type [list X], and a natural number
  [n], if the length of [l] is [n], then [@nth_error X l n] will return [None].

  Proof by induction on [n].

  - If [n] is [O], then [l] must be [nil] based on the definition of [length]
  and the fact that [length l = n]. [nth_error X nil n] then simplifies down to
  [None].

  - Assume [length l' = n' -> @nth_error X l' n' = None] and [n] is [S n'].
  [length l = S n'], which implies that [l = cons x l'].
  [@nth_error X l n = @nth_error X l' n'] by definition.
  [@nth_error X l' n' = None] by the induction hypothesis. Therefore, the
  theorem is proved true.
...
Definition scc (n : cnat) : cnat :=
  ...
  fun (X : Type) (succ : X -> X) (zero : X) => (n X succ (succ zero)).
...
Definition exp (n m : cnat) : cnat :=
  fun (X : Type) (succ : X -> X) (zero : X) =>
    (m (X -> X) (n X) succ) zero.
    (* n X (fun x => (mult m m) X succ x) (succ zero). *)
    (* n X (fun x => m X (fun x => m X succ x) x) zero. *)
    (* m X (fun x => n X (fun x => n X succ x) x) (succ zero). *)
