...
Fixpoint at_least_100 (n : nat) : nat :=
  if 100 <=? n then
    n
  else
    at_least_100 (n + 1).
...
Proof.
  intros late_days g.
  intros H_lower_bound H_upper_bound.
  rewrite -> apply_late_policy_unfold.
  rewrite -> H_lower_bound.
  rewrite -> H_upper_bound.
  reflexivity.
Qed.
...
Check (some nat 42) : option nat.
Check none nat : option nat.

Definition safe_sub {A : Type} (a b : nat) : option nat :=
  if a <=? b then
    some nat (a - b)
  else
    none nat.
...
Theorem option_monad_left_identity :
  forall (A B : Type),
  forall (x : A),
  forall (f : A -> option B),
  bind (ret x) f = f x.
...
Theorem option_monad_right_identity :
  forall (A B : Type),
  forall (x : A),
  forall (f : A -> option B),
  bind (f x) ret = f x.
Proof.
  ...
  intros x f.
  simpl.
  destruct (f x) eqn:E_fx.
    - reflexivity.
    - reflexivity.
Qed.

Theorem option_monad_associativity :
  forall (A B C : Type),
  forall (m : option A),
  forall (f : A -> option B),
  forall (g : B -> option C),
  bind (bind m (fun x => f x)) g = bind m (fun x => bind (f x) g).
Proof.
  intros A B C.
  intros m f g.
  simpl.
