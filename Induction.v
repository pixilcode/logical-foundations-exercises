...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    - First, suppose that [n = 0]. We must show that

        0 + m = m + 0

      This follows directly from the definition of [+].
    
    - Next, suppose that [n = S n'], where
...
Theorem mul_distr_r : forall n k : nat,
  n * (S k) = n + n * k.
Proof.
  intros n k.
  induction n as [|n' IHn'].
...
Proof.
  ...
  (* assert (mult_plus_distr_r : forall a b, S a * b = a * b + b).
    {
      intros a b.
      simpl.
      rewrite add_comm.
...
(*   
  assert (mult_1_comm : forall m, m * 1 = 1 * m).
    {
      intros m.
      simpl.
      rewrite -> add_0_r.
  ...
  assert (mult_2_comm : forall m, m * 2 = 2 * m).
    {
      intros m.
      simpl.
      rewrite -> add_0_r.
  ...
  assert (mult_3_comm : forall m, m * 3 = 3 * m).
    {
      intros m.
      simpl.
      rewrite -> add_0_r.
      ...
      - simpl.
        rewrite -> add_comm.
        simpl.
        assert (H : m' + S m' = S m' + m'). { apply add_comm. }
        rewrite -> H.
        simpl.
        rewrite -> IHm'.
        rewrite -> add_assoc.
  ...
  assert (H1 : forall n, S n = 1 + n).
    { reflexivity. }
  
  assert (distribute : forall n k, n * S k = n + n * k).
    {
      intros n k.
...
(* This will require just simplification *)
...
(* This will require case analysis *)
...
(*
  This will require induction
  (FALSE: the definition of equality simplifies this
          case to false)
*)
...
(*
  This will require simplification
  (FALSE: multiplication simplifies, but the term it
          simplifies to, [n + 0 = 0], requires
          induction)
*)
...
(* This will require induction on n *)
...
Theorem mult_distr_three : forall n m p : nat,
  p * m + p * (n * m) = p * (m + n * m).
Proof.
intros n m p.
...
Proof.
  ...
  - simpl.
    ...
    assert (H1 : m * p = p * m). { apply mul_comm. }
    assert (H2 : (m + n' * m) * p = p * (m + n' * m)). { apply mul_comm. }
    assert (H3 : (n' * m) * p = p * (n' * m)). { apply mul_comm. }

    rewrite H1.
    rewrite H2.
    rewrite H3.

    rewrite mult_distr_three.
    reflexivity.
    (* m * p + n' * m * p = (n' * m + m) * p *)
  (* induction m as [|m' IHm'].
  - simpl.
    ...
    reflexivity.
  - simpl.
    assert (H : forall a b c, a * (b + c * b) = a * b + a * c * b).
      {
        intros a b c.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    replace (bin_to_nat (incr b') + 0) with (bin_to_nat (incr b')).
    replace (bin_to_nat b' + 0) with (bin_to_nat b').
    rewrite IHb'.
    rewrite add_assoc.
    replace ((1 + bin_to_nat b') + 1) with (1 + (1 + bin_to_nat b')).
    simpl.
    reflexivity.
    
    (* bin_to_nat (incr b') + 0 => bin_to_nat (incr b') *)
    apply add_comm.

    (* bin_to_nat b' + 0 => bin_to_nat b' *)
    rewrite add_comm. reflexivity.

    (* (1 + bin_to_nat b') + 1 => 1 + (1 + bin_to_nat b') *)
    rewrite add_comm. reflexivity.
Qed.
...
(*
  [B0 Z] is a completely valid, but seperate way to write [Z]. In fact,
  you can add any number of [B0]'s to the [Z], and it will still equal 0.
  Basically, adding [B0]'s after [Z] is the equivalent of padding a
  binary number with [0]'s on the right. ([1001 = 001001])
*)
...
Example normalize_0 : Z = normalize Z.
Proof. reflexivity. Qed.

Example normalize_1 : B1 Z = normalize (B1 Z).
Proof. reflexivity. Qed.

Example normalize_2 : B0 (B1 Z) = normalize (B0 (B1 Z)).
Proof. reflexivity. Qed.

Example normalize_padded_1 : B1 Z = normalize (B1 (B0 Z)).
Proof. reflexivity. Qed.
...
Lemma double_distr : forall (b : bin) (f : bin -> nat),
  double (f b) = f b + f b.
Proof.
  intros b f.
  induction (f b) as [|b'].
...
Lemma double_incr_bin_distr : forall (n : nat) (f : nat -> bin),
  double_bin (incr (f n)) = incr (incr (double_bin (f n))).
Proof.
  intros n f.
  induction (f n); apply double_incr_bin.
Qed.

Lemma double_to_double_bin : forall (b : bin),
  nat_to_bin (double (bin_to_nat b)) = double_bin (nat_to_bin (bin_to_nat b)).
Proof.
  intros b.
  induction (bin_to_nat b) as [|b'].
  - reflexivity.
  - simpl. 
    rewrite IHb'.
    rewrite double_incr_bin_distr.
    reflexivity.
Qed.

Lemma incr_double_bin_to_B1 : forall (b : bin) (f : bin -> bin),
  incr (double_bin (f b)) = B1 (f b).
Proof.
  intros b f.
  destruct (f b) as [|b'|b']; reflexivity.
Qed.
...
Proof.
  ...
  - simpl.
    rewrite add_0_r.
    rewrite <- double_distr.
    rewrite double_to_double_bin.
    rewrite IHb'.
    reflexivity.
  - simpl. 
    rewrite add_0_r.
...
(* NOTES:
  - Question: is it possible to require an [Instance] of a [Class] to prove
    certain things? (Such as in the case of the monad laws)
*)
...
Theorem off_book :
  forall (a b : nat), add a b = add' a b.

*)

Definition double_is_even (n : nat) : Prop := even (double n) = true.

Definition less_than_5 (n : nat) : Prop := n <=? 5 = true.

Definition even_prop (n : nat) : Prop := even n = true.

Theorem ind_nat : (* "Type declaration" *)
  forall (P : nat -> Prop),
    (P 0) ->
    (forall (n : nat), P n -> P (S n)) ->
    forall (n : nat), P n.
Fixpoint induction_on_nats (* "Proof" *)
  (P : nat -> Prop)
  (proof_of_0 : P 0)
  ...
  match n with
  | 0 => proof_of_0
  | S n' => IH n' (induction_on_nats P proof_of_0 IH n')
  end.

Check induction_on_nats.

Lemma something_else : forall (n : nat),
  (0 <=? n = true) -> 0 <=? (S n) = true.
Proof. reflexivity. Qed.

Theorem something : forall (n : nat),
  0 <=? n = true.
Proof.
  apply induction_on_nats.
  - reflexivity.
  - apply something_else.
Qed.
...
Theorem ind_bin :
  forall (P : bin -> Prop),
    (P Z) ->
    (forall (n : bin), P n -> P (B0 n)) ->
    (forall (n : bin), P n -> P (B1 n)) ->
    forall (b : bin), P b.
Proof. Admitted.